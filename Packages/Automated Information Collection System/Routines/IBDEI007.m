IBDEI007 ; ; 04-JAN-2011
 ;;3.0;IB ENCOUNTER FORM IMP/EXP;;JAN 04, 2011
 Q:'DIFQ(358.3)  F I=3D1:2 S X=3D$T(Q+I) Q:X=3D""  S
Y=3D$E($T(Q+I+1),4,999),X=3D$E(X,4,999) S:$A(Y)=3D126
I=3DI+1,Y=3D$E(Y,2,999)_$E($T(Q+I+1),5,99) S:$A(Y)=3D61 Y=3D$E(Y,2,999) =
X NO E
S @X=3DY
Q Q
 ;;^DIC(358.3,0,"GL")
 ;;=3D^IBE(358.3,
 ;;^DIC("B","IMP/EXP SELECTION",358.3)
 ;;=3D
 ;;^DIC(358.3,"%D",0)
 ;;=3D^^4^4^2940217^
 ;;^DIC(358.3,"%D",1,0)
 ;;=3D=20
 ;;^DIC(358.3,"%D",2,0)
 ;;=3DThis file is nearly identical to file #357.3. It is used by the
 ;;^DIC(358.3,"%D",3,0)
 ;;=3DImport/Export Utility as a temporary staging area for data from =
that
file
 ;;^DIC(358.3,"%D",4,0)
 ;;=3Dthat is being imported or exported.
 ;;^DD(358.3,0)
 ;;=3DFIELD^^3^15
 ;;^DD(358.3,0,"DDA")
 ;;=3DN
 ;;^DD(358.3,0,"DT")
 ;;=3D2961031
 ;;^DD(358.3,0,"ID",.03)
 ;;=3DS
%I=3DY,Y=3D$S('$D(^(0)):"",$D(^IBE(358.2,+$P(^(0),U,3),0))#2:$P(^(0),U,1)=
,1:
""),C=3D$P(^DD(358.2,.01,0),U,2) D Y^DIQ:Y]"" W "
",Y,@("$E("_DIC_"%I,0),0)") S Y=3D%I K %I
 ;;^DD(358.3,0,"ID",.04)
 ;;=3DS
%I=3DY,Y=3D$S('$D(^(0)):"",$D(^IBE(358.4,+$P(^(0),U,4),0))#2:$P(^(0),U,1)=
,1:
""),C=3D$P(^DD(358.4,.01,0),U,2) D Y^DIQ:Y]"" W "
",Y,@("$E("_DIC_"%I,0),0)") S Y=3D%I K %I
 ;;^DD(358.3,0,"IX","APO",358.3,.03)
 ;;=3D
 ;;^DD(358.3,0,"IX","APO1",358.3,.04)
 ;;=3D
 ;;^DD(358.3,0,"IX","APO2",358.3,.05)
 ;;=3D
 ;;^DD(358.3,0,"IX","B",358.3,.01)
 ;;=3D
 ;;^DD(358.3,0,"IX","C",358.3,.03)
 ;;=3D
 ;;^DD(358.3,0,"IX","D",358.3,.04)
 ;;=3D
 ;;^DD(358.3,0,"NM","IMP/EXP SELECTION")
 ;;=3D
 ;;^DD(358.3,0,"VRPK")
 ;;=3DIBD
 ;;^DD(358.3,.01,0)
 ;;=3DSELECTION ID^RF^^0;1^K:$L(X)>30!($L(X)<3)!'(X'?1P.E) X
 ;;^DD(358.3,.01,1,0)
 ;;=3D^.1
 ;;^DD(358.3,.01,1,1,0)
 ;;=3D358.3^B
 ;;^DD(358.3,.01,1,1,1)
 ;;=3DS ^IBE(358.3,"B",$E(X,1,30),DA)=3D""
 ;;^DD(358.3,.01,1,1,2)
 ;;=3DK ^IBE(358.3,"B",$E(X,1,30),DA)
 ;;^DD(358.3,.01,3)
 ;;=3DAnswer must be 3-30 characters in length.
 ;;^DD(358.3,.01,21,0)
 ;;=3D^^2^2^2930309^
 ;;^DD(358.3,.01,21,1,0)
 ;;=3D=20
 ;;^DD(358.3,.01,21,2,0)
 ;;=3DThe ID passed by the package.
 ;;^DD(358.3,.01,"DT")
 ;;=3D2921119
