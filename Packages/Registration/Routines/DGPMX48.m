...
DE S DIE="^DGPM(",DIC=DIE,DP=405,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^DGPM(DA,-1))<0
 I $D(^("LD")) S %Z=^("LD") S %=$P(%Z,U,1) S:%]"" DE(1)=% S %=$P(%Z,U,2) S:%]"" DE(2)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGPMX48",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="LD;1",DV="RP406.41'",DU="",DLB="REASON FOR LODGING",DIFLD=30.01
 S DU="DG(406.41,"
 G RE
X1 Q
2 S DW="LD;2",DV="F",DU="",DLB="LODGING COMMENTS",DIFLD=30.02
 G RE
X2 K:$L(X)>30!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
