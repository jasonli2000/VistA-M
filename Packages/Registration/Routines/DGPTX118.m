DGPTX118 ; ;06/24/93
 D DE G BEGIN
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=2,DIEL=0,DU="" K DG,DE,DB Q:$N(^DPT(DA,-1))<0
 I $D(^(.321)) S %Z=^(.321) S %=$P(%Z,U,12) S:%]"" DE(1)=%
 I $D(^(.52)) S %Z=^(.52) S %=$P(%Z,U,5) S:%]"" DE(2)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGPTX118",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".321;12",DV="SX",DU="",DLB="RADIATION EXPOSURE METHOD",DIFLD=.3212
 S DU="N:NAGASAKI/HIROSHIMA;T:NUCLEAR TESTING;B:BOTH;"
 G RE
X1 S DFN=DA D IR^DGLOCK Q
 Q
 ;
2 S DW=".52;5",DV="RSX",DU="",DLB="POW STATUS INDICATED?",DIFLD=.525
 S DE(DW)="C2^DGPTX118"
