...
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=2,DIEL=0,DU="" K DG,DE,DB Q:$N(^DPT(DA,-1))<0
 I $D(^(.11)) S %Z=^(.11) S %=$P(%Z,U,6) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGPTX121",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW=".11;6",DV="F",DU="",DLB="ZIP CODE",DIFLD=.116
 S DE(DW)="C1^DGPTX121"
 G RE
...
C1S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 S A1B2TAG="PAT" D ^A1B2XFR
 Q
X1 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>5!($L(X)<5)!'(X?5N) X
 I $D(X),X'?.ANP K X
 Q
 ;
2 D:$D(DG)>9 F^DIE17 G ^DGPTX122
