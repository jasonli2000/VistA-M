...
DE S DIE="^DGPT(D0,""S"",",DIC=DIE,DP=45.01,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$N(^DGPT(D0,"S",DA,-1))<0
 I $D(^(300)) S %Z=^(300) S %=$P(%Z,U,1) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGPTX412",DQ=1
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="300;1",DV="SX",DU="",DLB="KIDNEY SOURCE",DIFLD=300.01
 S DU="1:Live Donor;2:Cadavar;"
 G RE
X1 S DGFLAG=1 D 401^DGPTSC01 K:DGER X K DGER,DGFLAG
 Q
 ;
2 S DQ=3 ;@899
3 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=3 D X3 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X3 K DGPTIT S Y=DGNFLD
 Q
4 G 1^DIE17
