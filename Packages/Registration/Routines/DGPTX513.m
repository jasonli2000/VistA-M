...
DE S DIE="^DGPT(D0,""M"",",DIC=DIE,DP=45.02,DL=2,DIEL=1,DU="" K DG,DE,DB Q:$N(^DGPT(D0,"M",DA,-1))<0
 I $D(^(300)) S %Z=^(300) S %=$P(%Z,U,3) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGPTX513",DQ=1
1 S DW="300;3",DV="SX",DU="",DLB="LEGIONNAIRE'S DISEASE",DIFLD=300.03
 S DU="1:Yes;2:No;"
 G RE
...
X4 I '$D(DGBPC(4))!(DGDUP(4)) S Y="@830"
 Q
5 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=5 D X5 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X5 D DRUG^DGPTSC01 I $D(DGTX) S Y="@825"
 Q
6 D:$D(DG)>9 F^DIE17 G ^DGPTX514
