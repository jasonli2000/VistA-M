...
DE S DIE="^DGPT(",DIC=DIE,DP=45,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$N(^DGPT(DA,-1))<0
 I $D(^(300)) S %Z=^(300) S %=$P(%Z,U,6) S:%]"" DE(1)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
I I DV'["I",DV'["#" G RD
 ...
 Q
BEGIN S DNM="DGPTX717",DQ=1
1 S DW="300;6",DV="NJ2,0X",DU="",DLB="CURRENT FUNCTIONAL ASSESSMENT",DIFLD=300.06
 G RE
X1 S DGFLAG=6 D 701^DGPTSC01 S:DGER X="" K DGER,DGFLAG K:+X'=X!(X>90)!(X<0)!(X?.E1"."1N.N) X
 Q
 ;
