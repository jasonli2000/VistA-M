DSIROIRV ;EWL - Document Storage System;Year End FOIA Report ;09/22/2009 13:15
 ...
 Q
 ;
FOIA(AXY,STDT,ENDT,DIV) ;RPC - DSIR YEAR END FOIA REPORT
 ; INPUT 
 ...
 ; The return array will have the following data with each piece separated by a "^":
 ; Piece Description
 ;  1 Request IEN
 ;  2 Patient/FOIA
 ;  3 Date Opened
 ...
 ; 
 N DLIST,NLIST ; THESE LINES ADDED TO CORRECTLY ESTALBLISH THE DIVISION(S)
 I $G(DIV)  S DIV=$P(DIV,U)
 I '$G(DIV) S DIV=DUZ(2)
 ;
 N AXYN,PAREQ,LODT,XRF,WRKD,FIL,STH,STH0,ROI,LXR,QGL,STAD,STDP,GBL,CTR,CNT,YY S YY=0
 N ROI0,ROI10,ROI12,ROI13,ROI4,ROI6,ST2,REQTY,FOPDT,FCLST,FCLDP,FCLDT
 S CNT=0,AXY=$NA(^TMP("DSIRYRFOIAV",$J))
 S STDT=+$G(STDT)\1,ENDT=+$G(ENDT)\1,AXYN="DSIRYRFOIAV",CTR=0
 ;
 D:$D(^DSIR(19620.3,"AFYDIV",ENDT,DIV))
 .S TST=0 F  S TST=$O(^DSIR(19620.3,"AFYDIV",ENDT,DIV,TST)) Q:'TST  S IEN=TST
 K @AXY,^TMP("DSIR",$J),^TMP("DSIR91",$J),^TMP("DSIR20",$J),^TMP("DSIROIRV",$J)
 ...
 F  S QGL=$Q(@QGL) Q:'($QS(QGL,2)=XRF)  S ROI=$QS(QGL,4) D:'$D(^TMP("DSIR20",$J,ROI,"V")) CHK
 ; 
 ; CHECK IF THE REQUEST IS STILL OPEN OR PENDING
 S XRF="AFOIA",QGL=$NA(^DSIR(19620,XRF))
 F  S QGL=$Q(@QGL) Q:'($QS(QGL,2)=XRF)  S ROI=$QS(QGL,3) D:'$D(^TMP("DSIR20",$J,ROI,"V")) CHK
 ;
...
CHK ;
 ...
 ;
TOTALS ;
 S ROI=0 F  S ROI=$O(^TMP("DSIROIRV",$J,ROI)) Q:'ROI  S XX=$G(^TMP("DSIROIRV",$J,ROI)) D
 .N GET,IENS,GLDX S IENS=ROI_",",GLDX="DSIROIRV"
 ...
 .I $P(ABC,U,5)'="NO" S ^TMP(AXYN,$J,ROI)=ABC Q
...
STATPROC ; STATISTICS FOR PERFECTED CLOSED REQUESTS
 I $$PERFCHK^DSIROIR0(0) D REAS(45) D
 .N WRKDT,WORKDAYS
 .S WRKDT=+$P(ROI10,U,6)
 .S WORKDAYS=$$EN^XUWORKDY(WRKDT,FCLDT)-$$PNDCLR^DSIROIR0(ROI,ENDT)-$$PAYPND^DSIROIR0(ROI,ENDT)
...
STATPEND ; STATISTICS FOR PENDING REQUESTS
 ...
 S WRKD=$$EN^XUWORKDY(OPDT,ENDT)-$$PNDCLR^DSIROIR0(ROI,ENDT)-$$PAYPND^DSIROIR0(ROI,ENDT)
 S:WRKD<1 WRKD=1
 I $$PERFCHK^DSIROIR0(1) D REAS(61),REAS(62,WRKD)
 Q
 ;*******
REAS(XX,YY) ;
