SDBT ; GENERATED FROM 'SDB' INPUT TEMPLATE(#102), FILE 44;01/05/11
 D DE G BEGIN
DE S DIE="^SC(",DIC=DIE,DP=44,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^SC(DA,""))=""
 ...
 I $D(^("LTR")) S %Z=^("LTR") S %=$P(%Z,U,1) S:%]"" DE(18)=% S %=$P(%Z,U,2) S:%]"" DE(19)=%
...
C12F1 N X,X1,X2 S DIXR=446 D C12X1(U) K X2 M X2=X D C12X1("O") K X1 M X1=X
 I $G(X(1))]"" D
 . K ^SC("AST",X,DA)
