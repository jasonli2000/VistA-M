...
DE S DIE="^SC(",DIC=DIE,DP=44,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^SC(DA,""))=""
 ...
 I $D(^("SL")) S %Z=^("SL") S %=$P(%Z,U,1) S:%]"" DE(5)=% S %=$P(%Z,U,2) S:%]"" DE(8)=% S %=$P(%Z,U,5) S:%]"" DE(2)=% S %=$P(%Z,U,6) S:%]"" DE(10)=% S %=$P(%Z,U,7) S:%]"" DE(3)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
X3 K:+X'=X!(X>9999)!(X<0)!(X?.E1"."1N.N) X
 ...
 ;
4 S D=0 K DE(1) ;1910
 S DIFLD=1910,DGO="^SDBT10",DC="1^44.03A^SI^",DV="44.03F",DW="0;1",DOW="SPECIAL INSTRUCTIONS",DLB=$P($$EZBLD^DIALOG(8042,DOW),": ") S:D DC=DC_D
...
X7 I '$D(SDLA) S SDLA=X
 Q
8 S DW="SL;2",DV="S",DU="",DLB="VARIABLE APP'NTMENT LENGTH",DIFLD=1913
 S DU="V:YES, VARIABLE LENGTH;"
 G RE
X8 Q
9 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=9 D X9 D:$D(DIEFIRE)#2 FIREREC^DIE17 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X9 S:+$O(^SC(DA,"ST",0))>0 Y="@99"
 Q
10 S DW="SL;6",DV="RSX",DU="",DLB="DISPLAY INCREMENTS PER HOUR",DIFLD=1917
 S DU="1:60-MIN ;2:30-MIN ;4:15-MIN ;3:20-MIN ;6:10-MIN ;"
 S Y="4"
 G Y
X10 S ZSI=$S(X=1!(X=2)!(X=3)!(X=4)!(X=6):60/X,1:0),SDLA=$S('$D(^SC(DA,"SL")):0,1:+^("SL")) K:('SDLA)!('ZSI) SDLA,ZSI,X Q:'$D(X)  I SDLA#ZSI>0 X ^DD(44,1917,9.2) Q
 Q
 ;
11 S DQ=12 ;@99
12 G 0^DIE17
