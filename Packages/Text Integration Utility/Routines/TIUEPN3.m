...
DE S DIE="^TIU(8925,",DIC=DIE,DP=8925,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^TIU(8925,DA,""))=""
 I $D(^(12)) S %Z=^(12) S %=$P(%Z,U,1) S:%]"" DE(10)=% S %=$P(%Z,U,2) S:%]"" DE(1)=% S %=$P(%Z,U,4) S:%]"" DE(4)=% S %=$P(%Z,U,8) S:%]"" DE(16)=%
...
X12 I '+$$REQCOSIG^TIULP(+$G(TIUTYP),+DA,+$G(TIUESNR)),($L($P($G(^TIU(8925,+DA,12)),U,8))'>0) S Y="@7"
 Q
13 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=13 D X13 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X13 I +$$ISA^USRLM(DUZ,"TRANSCRIPTIONIST") S Y="@8"
 Q
14 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=14 D X14 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X14 S TIUDCSNR=$$PERSNAME^TIULC1(+$P($G(TIUPREF),U,9))
 Q
15 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=15 D X15 G A:$D(Y)[0,A:Y=U S X=Y,DIC(0)="F",DW=DQ G OUT^DIE17
X15 I TIUDCSNR="UNKNOWN" S Y="@11"
 Q
16 D:$D(DG)>9 F^DIE17,DE S DQ=16,DW="12;8",DV="*P200'R",DU="",DLB="EXPECTED COSIGNER",DIFLD=1208
 S DE(DW)="C16^TIUEPN3"
 S DU="VA(200,"
 S X=TIUDCSNR
 S Y=X
...
C16S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X16 S DIC("S")="I +$$SCRCSNR^TIULA3(DA,+Y)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
