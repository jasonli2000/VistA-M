...
DE S DIE="^TIU(8925,",DIC=DIE,DP=8925,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^TIU(8925,DA,""))=""
 I $D(^(12)) S %Z=^(12) S %=$P(%Z,U,8) S:%]"" DE(1)=%,DE(4)=%
 I $D(^(13)) S %Z=^(13) S %=$P(%Z,U,3) S:%]"" DE(14)=%
 I $D(^(14)) S %Z=^(14) S %=$P(%Z,U,4) S:%]"" DE(8)=%
 I $D(^(15)) S %Z=^(15) S %=$P(%Z,U,6) S:%]"" DE(6)=%
 I $D(^(17)) S %Z=^(17) S %=$P(%Z,U,1) S:%]"" DE(10)=%
 K %Z Q
 ;
W W !?DL+DL-2,DLB_": "
 Q
...
1 D:$D(DG)>9 F^DIE17,DE S DQ=1,DW="12;8",DV="*P200'R",DU="",DLB="EXPECTED COSIGNER",DIFLD=1208
 S DE(DW)="C1^TIUEPN4"
 S DU="VA(200,"
 G RE
...
X2 S Y="@9"
 Q
3 S DQ=4 ;@8
4 D:$D(DG)>9 F^DIE17,DE S DQ=4,DW="12;8",DV="*P200'",DU="",DLB="EXPECTED COSIGNER",DIFLD=1208
 S DE(DW)="C4^TIUEPN4"
 S DU="VA(200,"
 G RE
...
C4S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 D SACLEC^TIUDD0(1208,X)
 Q
X4 S DIC("S")="I +$$SCRCSNR^TIULA3(DA,+Y)" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
5 S DQ=6 ;@9
6 D:$D(DG)>9 F^DIE17,DE S DQ=6,DW="15;6",DV="S",DU="",DLB="COSIGNATURE NEEDED",DIFLD=1506
 S DU="1:YES;0:NO;"
 S Y="1"
...
X6 Q
7 S DQ=8 ;@7
8 S DW="14;4",DV="P49'",DU="",DLB="SERVICE",DIFLD=1404
 S DE(DW)="C8^TIUEPN4"
 S DU="DIC(49,"
 S X=$P($G(TIU("SVC")),U,2)
 S Y=X
...
X9 I +$P($G(TIUPREF),U,8)'>0,($G(^TIU(8925,+DA,17))']"") S Y="@10"
 Q
10 D:$D(DG)>9 F^DIE17,DE S DQ=10,DW="17;1",DV="F",DU="",DLB="SUBJECT (OPTIONAL description)",DIFLD=1701
 S DE(DW)="C10^TIUEPN4"
 G RE
...
C10S S X="" Q:DG(DQ)=X  K DB S X=DG(DQ),DIC=DIE
 ...
 Q
X10 K:X[""""!($A(X)=45) X I $D(X) K:$L(X)>80!($L(X)<3) X
 I $D(X),X'?.ANP K X
 Q
 ;
11 S DQ=12 ;@10
...
X13 I $P($G(^TIU(8925,+DA,13)),U,3)]"" S Y="@12"
 Q
